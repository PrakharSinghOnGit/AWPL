...
async function MakeJsons() {
  files.forEach(async (file) => {
    ...
    if (file.includes("LEVEL")) {
      ...
      let levelHtml = MakeHtmlforLevel(
        SortedLevelData,
        "SP",
        file.replace(".json", ""),
        true
      ...
      let targetHtml = MakeHtmlforTarget(
        SortedLevelData,
        "SP",
        file.replace(".json", ""),
        true
...
function MakeHtmlforLevel(DATA, TYPE, FILENAME, removeEmpty = false) {
...
function MakeHtmlforTarget(DATA, TYPE, FILENAME, removeEmpty = false) {
  const style = `<style id="style">
  @page{margin: 0mm;}
  * {color: ${Setting.print.HeadingColor};
    margin: 0;
